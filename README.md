...
- 바로 문제 풀기 → 정답·영어 뜻·한국어 뜻·예문 확인 → 간격 복습(1·3·7·14·30·60일)
- `모르겠어요`를 누르면 정답 해설을 보여주고 오답과 같은 기준으로 기록합니다. 오답 자동 저장 설정을 따르며, 학습 묶음에 문제를 추가하지 않습니다.
- 문제·보기 읽기, 정답·예문 읽기, 단어장 음성 읽기. 설정에서 자동 읽기(기본 꺼짐)와 속도(0.75~1.5배) 조절
...
학습 탭은 진행 중인 문제가 있어도 학습 메뉴를 엽니다. `이어서 풀기`로 저장된 문제로 돌아갈 수 있고, `오늘의 복습`·`틀린 문제만`도 선택할 수 있습니다. 다른 학습으로 바꾸기 전 이미 푼 문제의 기록은 유지합니다.

추가된 단어장은 학습 중에도 자동 반영하며, 풀던 문제와 문제 수는 유지합니다. 업데이트로 남은 문제의 단어가 삭제되는 경우에만 해당 묶음을 마칠 때까지 적용을 미룹니다. 새 앱 버전이 준비되면 화면의 `지금 적용` 버튼으로 기록을 저장하고 업데이트할 수 있습니다. 1.2.0 이하 앱에서 처음 넘어올 때는 온라인에서 새로고침한 뒤 모든 워드루프 창을 닫고 다시 열어 주세요. 브라우저 데이터를 지울 필요는 없습니다.
...
모바일 문제 화면은 별도의 음성 버튼 줄 없이 문제·보기·모르겠어요를 우선 배치합니다. 긴 문제나 큰 글자 설정에서는 내용을 자르지 않고 스크롤합니다.

### 음성 읽기

브라우저의 Web Speech API(`speechSynthesis`)를 사용하며 별도 API 키나 유료 서비스 연동은 필요 없습니다. 기기에 설치된 영어 음성을 우선 사용합니다. 제공되는 목소리와 오프라인 재생 여부는 브라우저·기기의 음성 엔진에 따라 다릅니다. 온라인 음성은 읽을 영어 텍스트를 해당 음성 서비스에서 처리할 수 있습니다.

문제 카드 오른쪽의 스피커는 현재 문제와 예문을 읽고, A~D 보기 오른쪽의 스피커는 해당 보기만 읽습니다. 보기의 답 선택 영역과 음성 버튼은 분리되어 있어 음성을 들어도 채점되지 않습니다. 아직 답하지 않은 빈칸은 `blank`로 읽으며 정답을 채우지 않습니다. 답한 뒤 해설 오른쪽의 스피커로 정답과 완성된 예문을 들을 수 있습니다. 읽는 아이콘은 중지 아이콘으로 바뀝니다. 같은 버튼을 다시 누르면 중지하고, 답 선택·화면 이동·다음 문제에서는 이전 읽기를 취소합니다.

자동 읽기를 켜면 학습 시작·이어하기·다음 문제에서 문제와 예문을 읽습니다. 페이지를 새로고침했다고 임의로 재생하지 않으며, 브라우저가 자동 재생을 막으면 읽기 버튼을 직접 눌러 시작할 수 있습니다.
...
새로고침 이어하기, 오프라인 실행, 개인 단어, 오답 선택, 백업 복원, 콘텐츠 업데이트 및 실패 시 보존, 320/390/1440px 화면과 동시 탭 보호를 확인합니다. 음성 통합 검사는 모의 음성 엔진으로 정답 노출 방지, 버튼·자동 읽기·중지, 설정 저장 및 미지원 환경을 검증합니다. 실제 음성 품질과 기기별 재생은 해당 기기에서 확인해야 합니다. 스크린샷은 `test-results/`에 생성되며 Git에 포함하지 않습니다.
...
외부 폰트, 추적 스크립트, API 키를 사용하지 않습니다. 학습 기록은 기기에 보관합니다. 음성 읽기는 기기 또는 브라우저가 제공하는 음성 서비스를 사용합니다.
