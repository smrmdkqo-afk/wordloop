...
`문제 듣기`는 현재 문제와 예문을, `보기 듣기`는 화면에 나온 A~D 순서대로 보기를 읽습니다. 아직 답하지 않은 빈칸은 `blank`로 읽으며 정답을 채우지 않습니다. 답한 뒤 `정답·예문 듣기`로 완성된 문장을 들을 수 있습니다. 같은 버튼을 다시 누르면 중지하고, 답 선택·화면 이동·다음 문제에서는 이전 읽기를 취소합니다.
